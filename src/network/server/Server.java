...
public class Server {
		...
		private void authenticate(final Socket socket, final ObjectInputStream in, final ObjectOutputStream out) {
			Thread authentication = new Thread("Authentication thread") {
				public void run() {
					...
					try {
						out.writeObject(Command.HANDSHAKE);
						force(out);
						Object handShake = in.readObject();

						if (handShake == Command.HANDSHAKE) {
						...
						if (ctc != null && (clientLimit < 0 || clientLimit > clients.size())) {
							clients.put(id, ctc);
							out.writeObject(Command.CONNECTED);
							force(out);
						} else {
							// includes, handShake != ClientCommand.HANDSHAKE
							if (!socket.isClosed()) {
								out.writeObject(Command.REJECT_CONNECTION);
								force(out);
							}
							socket.close();
							return;
						...
						try {
							if (!socket.isClosed())
								out.writeObject(Command.ERROR_CONNECTION);
							in.close();
							out.close();
						} catch (IOException e) {
						}
	...
	public class ConnectionToClient {
		...
		public void localShutDown() {
			...
			try {
				if (!socket.isClosed())
					out.writeObject(Command.DISCONNECTED);
			} catch (IOException e) {
			}
			try {
				in.close();
